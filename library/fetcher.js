...
var latLngKey = 'latlng=';
var googleKey = '&key=AIzaSyDwxq6Bn-5cI5yisS1mxUch4VfSrwaNHfQ';
...
const DEMOGRAPHICS_URL = 'http://popdata.unhcr.org/api/stats/demographics.json?';
const ASYLUM_SEEKERS_URL = 'http://popdata.unhcr.org/api/stats/asylum_seekers.json?';
...
var fetcher = {
  ...
  personsOfConcern: function (year, countryId) {
    ...
    return new Promise((resolve, reject) => {
      ...
      var urlWithoutPage = PERSONS_OF_CONCERN_URL + yearKey + year + corKey + countryId + coo + page;
  ...
  demographics: (year, countryId) => {
    var pageNumber = 1;
    var urlWithoutPage = DEMOGRAPHICS_URL + yearKey + year + corKey + countryId + page;
    var url = urlWithoutPage + pageNumber;
    return new Promise((resolve, reject) => {
      fetch(url).then((res) => res.json()).then((res) => {
        var promises = [];
  ...
  asylumSeekers: (year, countryId) => {
    var pageNumber = 1;
    var urlWithoutPage = ASYLUM_SEEKERS_URL + yearKey + year + coa + countryId + coo + page;
    var url = urlWithoutPage + pageNumber;
    return new Promise((resolve, reject) => {
      fetch(url).then((res) => res.json()).then((res) => {
        var promises = [];
  ...
  getUserCountry: (lat, lng) => {
    var url = LOCATION_URL + latLngKey + lat + ',' + lng + googleKey
    return new Promise((resolve, reject) => {
      fetch(url).then((res) => res.json()).then((res) => {
        console.log(res);
      }).done();
